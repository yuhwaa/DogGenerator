...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        //Turn the object into and array
        const breedsArray=Object.keys(breedsObject)
        for (let i=0; i<breedsArray.length; i++) {
            //create <option></option> in the html
            const option = document.createElement('option')
            //<option value='breed'>
            option.value = breedsArray[i]
            //<option value='breed'>Breed</option>
            option.innerText = breedsArray[i]
            //adds current <option> tag to the select box list of options
    ...
    const getDogImage = url => {
        ...
        fetch(url)
            .then(res => {
                //get JSON message back
                return res.json();
            })
            .then(data => {
                //extract message from JSON and attach  to img tag as new src
    ...
    const dogInfo = {
        ...
        assignMF() {
            ...
            document.getElementById("MF").innerHTML = `Gender: ${this.MF}`
        },
        assignName(array) {
